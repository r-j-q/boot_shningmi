...
class Order extends Base
{
    ...
    public function changeFee($id = 0)
    {
        ...
        $order->order_sn = \app\admin\model\shopro\order\Order::getSn($order->user_id);     // 更新订单号，防止微信支付报错：商户订单号重复
        ...
        \addons\shopro\model\OrderAction::operAdd($order, null, $this->auth->getUserInfo(), 'admin', "订单改价：订单号由：" . $old_order_sn . "改为：" . $order->order_sn);
